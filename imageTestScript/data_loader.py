...
class DataLoader():
    ...
    def prepare_datas(self, shuffle=True):
        ...
        for dir_path in dir_paths:
            ...
            for img_path in files:
                ...
                #gt_path = 1
                #gt_path = self.get_gt_image_path(img_path)
    ...
    def get_minibatch(self, shuffle=True):
        ...
        if cf.Variable_input:
            imgs = np.zeros((mb, 3, cf.Max_side, cf.Max_side), dtype=np.float32)
        else:
        ...
        for i, ind in enumerate(self.mb_inds):
            ...
            img, height, width = self.image_dataAugment(
    ...
    def get_gt(self, img_name):
        #print(img_name)
        imgLabel = os.path.basename(img_name).split("_")[0]
        for ind, cls in enumerate(cf.Class_label):
            if imgLabel in cf.replaceChr:
                nm = cf.replaceChr.index(imgLabel)
                return cf.Class_label.index(cf.ngNameChr[nm])
            elif cls == imgLabel:
                return ind
    ...
    def get_gt_image_path(self, img_name):
        ...
        for gt_dir in gt_dirs:
            ...
            if os.path.exists(gt_path):
                return gt_path
        #for gt_dir in gt_dirs:
        #    for extension in cf.File_extensions:
